...
def gen_acceleration_graph(count):
    ...
    graph_data = rawdata[start:end]


    if start != end and count*TIME_PERIOD/1000 <= DISPLAY_RANGE:
        ...
        diff_df = pd.DataFrame(data={rawdata.columns[0]:diff_range.values, 
                                     rawdata.columns[1]: [None] * diff_range.shape[0], 
                                     rawdata.columns[2]: [None] * diff_range.shape[0], 
                                     rawdata.columns[3]: [None] * diff_range.shape[0]})
